...
def affine_transform_loss(P, P_prime, S, t):
    ...
    pred = np.dot(P, S) + t
    loss = np.mean(np.sum((pred-P_prime)**2, axis = 1))
    ...
    grad_S = 2* np.dot(P.T, pred-P_prime) / P.shape[0]
    grad_t = 2* np.mean(pred-P_prime, axis = 0)
...
def fit_affine_transform(P, P_prime, logger, learning_rate, steps):
    ...
    for i in range(steps):
        ...
        loss, pred, grad_S, grad_t = affine_transform_loss(P, P_prime, S, t)
        logger.log(i, loss, pred)
